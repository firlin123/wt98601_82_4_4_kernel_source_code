...
void SP0718_Sensor_Init(void)
{
	...
	SP0718_write_cmos_sensor(0xd7,0x30);//10//2x根据增益判断轮廓阈值
	...
	SP0718_write_cmos_sensor(0xe8,0x40);//48//轮廓强度
	...
	//dns       
	SP0718_write_cmos_sensor(0xfd,0x01);
	SP0718_write_cmos_sensor(0x64,0x88);//沿方向边缘平滑力度  //0-最强，8-最弱
	SP0718_write_cmos_sensor(0x65,0x44);
	SP0718_write_cmos_sensor(0x6d,0x02);//8//强平滑（平坦）区域平滑阈值
	...
	SP0718_write_cmos_sensor(0x75,0x66);//[7:4]平坦区域强度，[3:0]非平坦区域强度；0-最强，8-最弱；
...
UINT32 SP0718_Open(void)
{
	...
	//判断手机对应目录下是否有名为sp0718_sd 的文件,没有默认参数
	...
	//set_fs(fs);
	if(fromsd_sp0718 == 1)//是否从SD读取//gepeiwei   120903
		SP0718_Initialize_from_T_Flash();//从SD卡读取的主要函数
	else
		SP0718_Sensor_Init();
#else
	SP0718_Sensor_Init();
