...
static int check_otp_info(int index)
{
	...
	OV8858_write_cmos_sensor(0x3d88, (nFlagAddress>>8) & 0xff);
	OV8858_write_cmos_sensor(0x3d89, nFlagAddress & 0xff);
	// partial mode OTP write end address
	OV8858_write_cmos_sensor(0x3d8A, (nFlagAddress>>8) & 0xff );
	OV8858_write_cmos_sensor(0x3d8B, nFlagAddress & 0xff );
	// read otp into buffer
	OV8858_write_cmos_sensor(0x3d81, 0x01);
	mDELAY(5);
	flag = OV8858_read_cmos_sensor(nFlagAddress);
	//select group
	if (index == 1)
	{
	flag = (flag>>6) & 0x03;
	...
	// clear otp buffer
	OV8858_write_cmos_sensor(nFlagAddress, 0x00);
	if (flag == 0x00) {
	return 0;
	}
	else if (flag & 0x02) {
...
static int read_otp_info(int index, struct otp_struct *otp_ptr)
{
	...
	OV8858_write_cmos_sensor(0x3d81, 0x01);
	mDELAY(5);
	(*otp_ptr).module_integrator_id = OV8858_read_cmos_sensor(start_addr);
	(*otp_ptr).lens_id = OV8858_read_cmos_sensor(start_addr + 1);
	(*otp_ptr).production_year = OV8858_read_cmos_sensor(start_addr + 2);
	(*otp_ptr).production_month = OV8858_read_cmos_sensor(start_addr + 3);
	(*otp_ptr).production_day = OV8858_read_cmos_sensor(start_addr + 4);
	// clear otp buffer
...
static int read_otp_wb(int index, struct otp_struct * otp_ptr)
{
	...
	(*otp_ptr).rg_ratio = (OV8858_read_cmos_sensor(start_addr)<<2) + ((temp>>6) & 0x03);
	(*otp_ptr).bg_ratio = (OV8858_read_cmos_sensor(start_addr + 1)<<2) + ((temp>>4) & 0x03);
	(*otp_ptr).light_rg = (OV8858_read_cmos_sensor(start_addr + 2) <<2) + ((temp>>2) & 0x03);
	(*otp_ptr).light_bg = (OV8858_read_cmos_sensor(start_addr + 3)<<2) + (temp & 0x03);
	// clear otp buffer
	for (i=start_addr; i<=end_addr; i++) {
	OV8858_write_cmos_sensor(i, 0x00);
	}
...
static int read_otp_VCM(int index, struct otp_struct * otp_ptr)
{
	...
	temp = OV8858_read_cmos_sensor(start_addr+2);
	(* otp_ptr).VCM_start = (OV8858_read_cmos_sensor(start_addr)<<2) | ((temp>>6) & 0x03);
	(* otp_ptr).VCM_end = (OV8858_read_cmos_sensor(start_addr + 1) << 2) | ((temp>>4) & 0x03);
...
static int read_otp_lenc(int index, struct otp_struct * otp_ptr)
{
	...
	(* otp_ptr).lenc[i] = OV8858_read_cmos_sensor(start_addr + i);
	
	OV8858DB("OTP  i is : %d, lenc is : 0x%x \n ",i,(* otp_ptr).lenc[i]);
...
static int update_lenc(struct otp_struct * otp_ptr)
{
	...
	for(i=0;i<OTP_DRV_LSC_SIZE ;i++) {
	OV8858_write_cmos_sensor(OTP_DRV_LSC_REG_ADDR + i, (*otp_ptr).lenc[i]);
	}
	return 0;
}
...
static int update_otp_wb()
{
	...
	OV8858DB("R_gain=0x%x\n ",R_gain);
	OV8858DB("B_gain=0x%x\n ",B_gain);
	OV8858DB("G_gain=0x%x\n ",G_gain);
...
static int update_otp_lenc()
{
	...
	OV8858DB("###update_otp_lenc temp is : %d, i is :%d \n ",temp, i);
	if (temp == 2) {
	otp_index = i;
	break;}
	}
...
static kal_uint16 OV8858Gain2Reg(const kal_uint32 iGain)
{
	...
	iReg = iGain*OV8858_GAIN_BASE/BASEGAIN;
...
void OV8858_SetGain(UINT16 iGain)
{
	...
	#ifdef OV8858_DEBUG
	OV8858DB(" [OV8858_SetGain]ov8858.sensorGlobalGain=0x%x,ov8858.realGain =%x",ov8858.sensorGlobalGain,ov8858.realGain); 
	#endif
	//temperature test
...
void OV8858_1224pSetting(void)
{
	...
	OV8858_write_cmos_sensor(0x030e, 0x02); // ; pll2_rdiv //0x00
	OV8858_write_cmos_sensor(0x030f, 0x09); // ; pll2_divsp
	OV8858_write_cmos_sensor(0x0312, 0x03); // ; pll2_pre_div0, pll2_r_divdac 01
	OV8858_write_cmos_sensor(0x3015, 0x00); //01
...
OV8858_write_cmos_sensor(0x0302,0x1f);  // ; modify to 648Mbps/lane                                   
...
OV8858_write_cmos_sensor(0x3617,0xb0);//                                                                                
...
//csq
OV8858DB("[csq--]OV8858 0x4800 enter 0x24:\n ");
OV8858_write_cmos_sensor(0x4800,0x24);
//OV8858_write_cmos_sensor(0x4801,0x0f);
OV8858_write_cmos_sensor(0x4837,0x15);

//OV8858_write_cmos_sensor(0x4837,0x1a);//; global timing          
...
OV8858_write_cmos_sensor(0x5000,0x7e);//; slave AWB gain enable, slave AWB statistics enable, master AWB gain enable,   
OV8858_write_cmos_sensor(0x5001,0x01);//; BLC on                                                                        
OV8858_write_cmos_sensor(0x5002,0x08);//; H scale off, WBMATCH select slave sensor's gain, WBMATCH off, OTP_DPC         
...
UINT32 OV8858Open(void)
{
    ...
    OV8858_Init_Para();
	  ...
	  update_otp_lenc();
	  #ifdef OV8858_DEBUG
		  OV8858DB("[OV8858Open OTP] OTP done."); 
	  #endif	
...
UINT32 OV8858GetSensorID(UINT32 *sensorID)
{
	...
	mdelay(40);
	update_otp_wb();
	OV8858DB("%s Module ID: 0x%x ", __FUNCTION__,module_id);
	if(module_id != 0x7){ 
		*sensorID = 0xFFFFFFFF;
		OV8858DB("Read Sensor ID Fail = 0x%x\n", *sensorID);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
UINT32 OV8858SetTestPatternMode(kal_bool bEnable)
{
	...
	OV8858_write_cmos_sensor(0x5032, 0x04);
	OV8858_write_cmos_sensor(0x5033, 0x00);
	OV8858_write_cmos_sensor(0x5034, 0x04);
	OV8858_write_cmos_sensor(0x5035, 0x00);
	OV8858_write_cmos_sensor(0x5036, 0x04);
	OV8858_write_cmos_sensor(0x5037, 0x00);
    }
	else
	{
        OV8858_During_testpattern = KAL_FALSE;
		OV8858_write_cmos_sensor(0x5E00,0x00);
             OV8858_write_cmos_sensor(0x5000,0xfe);
	}
