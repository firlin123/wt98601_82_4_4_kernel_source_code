...
#if defined(OTP_SUPPORT)
   otp_lenc_update(0); //start  LSC' OTP func
#endif
    mDELAY(20);              // Allow PLL to lock
...
UINT32 A5142MIPIGetSensorID(UINT32 *sensorID) 
{
    ...
    kal_uint16 i;
    //kal_uint16 OTP_state = 0x0;	

    // A5142 may have different i2c address
    for(i = 0; i < sizeof(i2c_addr) / sizeof(i2c_addr[0]); i++)
    {
        spin_lock(&a5142mipiraw_drv_lock);
        A5142MIPI_sensor.i2c_write_id = A5142MIPI_WRITE_ID_1;//i2c_addr[i];
        spin_unlock(&a5142mipiraw_drv_lock);
...
UINT32 A5142MIPI_read0x3026(void)
{
	kal_uint32 adr=0;
    A5142MIPI_write_cmos_sensor(0x3026,0x7fff);
	mDELAY(10);
	adr=A5142MIPI_read_cmos_sensor(0x3026);
	SENSORDB("###A5142MIPI_read0x3026,adr=%x",adr);
}
EXPORT_SYMBOL(A5142MIPI_read0x3026);
