...
static void gsl_report_point(struct gsl_touch_info *ti)
{
	...
	else 
	{
		...
		for (tmp = 0; ti->finger_num > tmp; tmp++) 
		{
			...
			if(boot_mode!=NORMAL_BOOT && (strncmp(MTK_LCM_PHYSICAL_ROTATION, "270", 3)==0 || strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 3)==0) )
			{
				int temp;
				temp = ti->y[tmp];
				ti->y[tmp] = ti->x[tmp];
...
#endif
	if(strncmp(MTK_LCM_PHYSICAL_ROTATION, "270", 3)==0 || strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 3)==0)
	{
...
static int __devinit gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	
    {
		...
		hardwareinfo_set_prop(HARDWARE_TP, "gsl915");
	}
